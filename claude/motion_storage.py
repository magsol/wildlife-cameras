...
# API endpoints for remote server
"""
These endpoints should be implemented on the remote storage server to handle
chunked uploads from the Raspberry Pi camera.
...
async def init_chunked_upload(request: Request):
    ...
    chunk_size = data.get("chunk_size", MIN_UPLOAD_CHUNK_SIZE)
...
async def upload_chunk(request: Request):
    ...
    if not upload_id or not chunk_data:
        raise HTTPException(status_code=400, detail="Missing upload_id or chunk_data")
    ...
    try:
        ...
        logger.error(f"Error processing chunk {chunk_index} for upload {upload_id}: {e}")
...
async def finalize_upload(request: Request):
    ...
    try:
        ...
        start_time = datetime.datetime.fromisoformat(
            original_metadata.get("start_time", datetime.datetime.now().isoformat())
        )
